 ...
 * <p>
 * 线程的基础知识：
 * 1. 线程是什么? 线程是操作系统CPU调度的基本单位, 每个进程都有一个主线程执行入口, 但是会有多个子线程分别执行任务;
 * 操作系统会根据自身的线程调度算法来调度线程的执行, 会分配高速缓存来缓存线程的执行状态, 用于恢复线程上下文, 这就是
 * 线程的切换任务, 是十分耗费资源的(相对概念), 这就有个问题, 线程并不是越多越好, 越多线程时间片分配的越少, 调度切换
 * 的频率就快, 那么执行的相对任务就少了.
 * 2. 单核cpu使用多线程可以吗? 可以, 因为CPU只管计算, 而线程任务不都是计算 还设计到IO, 当IO的时候, 将线程切换到其他线程
 * 继续计算, 充分发挥CPU的利用率。
 * 3. 线程设置的数量：n=u*r*(1+w/c)
 * <p>
 * 多线程编程的同步和互斥:
 * 同步：一个线程的执行依赖另外一个线程的消息, 比如notify()唤醒等待执行的线程.
 * 互斥：一个进程的共享内存资源一个时刻只允许一个线程使用, 其他线程必须等待, 知道独占线程使用完毕释放资源.
 * <p>
 * <p>
 * 并发3大特性:
 ...
 * <p>
 * 内存读取： 块读, 会把需要的变量及其周围变量整块读入.
 * <p>
 *
 * 2> 有序性: 单例模式 创建对象
 * 机器指令可以乱序执行. 指令可以乱序, 但是都要保证单线程最终一致性.
 * 不要在构造方法中使用线程, 因为this可能半初始化, 线程就执行了.
 * DCL: double check lock 双重检验锁 应对并发数据不一致问题.
 *
 *
 * 3> 原子性:
 *
 *
 * 线程的执行权和线程的优先级没有直接关系, 取决于操作系统的调度算法.
 *
 * happen-before:
 * 单线程最终一致性:
 *
